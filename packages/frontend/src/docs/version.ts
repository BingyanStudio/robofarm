...
export const UPDATE_LOG: UpdateEntry[] = [
    {
        ...
        items: [
            '## 平衡性调整',
            '浇水时机更稳定: 缺水回合先**均匀分布**在整个生长周期, 再施加 **2 回合以内**的随机偏移, 且任意两个浇水回合不重合 (原为完全随机选取); 随机仍对玩家不可预测, 避免硬编码浇水回合',
            '## 服务器验证',
            '单人验证改为使用**固定的 5 个随机种子**各完整执行一局, 取**平均分 (向上取整)** 作为成绩; 缺水时机随机, 服务器得分可能与本地试玩略有不同, 但服务器端结果可复现',
            '验证结果与"我的成绩"同时展示总得分与 5 局分别得分; 每次提交保存 5 份回放, 下载时弹窗选择下载哪一局',
            '## 排行榜',
            'v2.x 排行榜只统计新式多局验证的提交 (旧式单局验证的成绩不再计入)',
            '## 文档与提示',
            '提交确认弹窗增加提示: 缺水时机随机 (非固定回合), 服务器使用固定种子验证',
            'llms.txt 与 MCP 文档向 AI 强调缺水时机随机性与服务器固定种子机制, 避免 Agent 硬编码浇水时机',
        ],
    },
    {
        version: '2.0.0',
        ...
        items: [
            ...
            '## 界面改进',
            '回放文件携带录制时的游戏版本号: 播放/统计时若版本不匹配则弹出警告',
