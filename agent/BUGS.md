...
- [x] Bug: 多人游戏提前退出，后续又去观战的话，会同时收到多条战斗结果提示
  - 修复: battle.ts 的路由跳转即 SPA 屏幕卸载, 此前从不关闭 WebSocket, 残留连接仍订阅着
    房间; 再进观战会连第二条, 同一房间的 match-end 广播同时送达两条连接, modal() 叠出多个。
    现在路由跳转时关闭连接 (hashchange 一次性监听), connect() 前也会先关旧连接双保险。
